...
export default function LoginScreen({ navigation }) {
  ...
  const handleForgotPassword = async () => {
    if (!email) {
      return Alert.alert(
        "Email Required",
        "Please enter your registered email address above first to recover your password.",
      );
    }
    ...
    try {
      ...
      Alert.alert(
        "Recovery Email Sent",
        res.data.message ||
          "We have sent your current login password to your email. You can now login using this password.",
      );
    } catch (err) {
      setIsSendingForgot(false);
      Alert.alert(
            ...
            <View>
              <View style={styles.headerContainer}>
                ...
                <Text style={styles.title}>
                  Welcome <Text style={{ color: "#8b5cf6" }}>Back</Text>
                </Text>
